...
from TokenCut.unsupervised_saliency_detection import dino

import TokenCut.unsupervised_saliency_detection.object_discovery as tokencut
import argparse
from TokenCut.unsupervised_saliency_detection import utils
from TokenCut.unsupervised_saliency_detection import bilateral_solver
import os
import clip
...
import TokenCut.unsupervised_saliency_detection.metric
...
def get_tokencut_binary_map(img_pth, backbone,patch_size, tau) :
    ...
    # print(f'feat: {feat.shape} {tensor.shape}')
    # exit()
...
def mask_segment_compose(org, mask, mask_color = [173, 216, 230]) :
    ...
    return Image.fromarray(rgba), mask_fg
...
for cid, folder in enumerate(class_folders):
    ...
    for seq_id ,sequence in enumerate(sequences):
        ...
        for img_name in tqdm(img_list) :
            print(f'Segmenting image: {img_name}')
            # if args.img_path is not None:
