...
mask_bfs = []
gt = []
img_folders = sorted(os.listdir(args.img_folder_path))
# print(f'img_fol: {img_folders}')
# exit()
for folder in img_folders:
    folder_path = os.path.join(args.img_folder_path, folder)
    if not os.path.isdir(folder_path):
        continue
    out_folder_path = os.path.join(args.out_dir, folder)
    # args.out_dir = out_folder_path
    if not os.path.exists(out_folder_path):
        os.mkdir(out_folder_path)
    img_list = sorted(os.listdir(folder_path))
    img_list = [os.path.join(folder_path, im) for im in img_list]
    ...
    for img_name in tqdm(img_list) :
        ...
        if count_vis != args.nb_vis :
            print(f'args.out_dir: {out_folder_path}, img_name: {img_name}')
            ...
            out_name = os.path.join(out_folder_path, img_name)
            out_lost = os.path.join(out_folder_path, img_name.replace('.jpg', '_tokencut.jpg'))
            out_bfs = os.path.join(out_folder_path, img_name.replace('.jpg', '_tokencut_bfs.jpg'))
            ...
            if args.gt_dir is not None :
                ...
                out_gt = os.path.join(out_folder_path, img_name.replace('.jpg', '_gt.jpg'))
