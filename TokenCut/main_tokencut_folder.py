...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_dir", type=str, default="outputs/coco", help="Output directory to store predictions and visualizations."
    )
    ...
    # -------------------------------------------------------------------------------------------------------
    # Dataset
    img_folder = '/media/u6566739/c42f350c-c236-4250-8574-4dcf0809a9e5/coco/train2017'
    voc_img_folder = '/media/u6566739/c42f350c-c236-4250-8574-4dcf0809a9e5/VOCdevkit/VOC2012/JPEGImages'
    xpaste_img_folder = '/home/users/u6566739/project-004/XPaste/output/youtube_vis'


    images_path = sorted(os.listdir(voc_img_folder))
    i = 0
    annotation_file = '/media/u6566739/c42f350c-c236-4250-8574-4dcf0809a9e5/coco/train2017_tokencut_ann'
    voc_annotation_file = '/media/u6566739/c42f350c-c236-4250-8574-4dcf0809a9e5/VOCdevkit/VOC2012/trainval_tokencut_ann'

    collect_pred = []
    collect_pred_dict = {'image_files': [], 'bboxes': []}
    for img_name in images_path:
        # print(f'image: {img_name}')
        # if i ==10:
        #     break
        i +=1
        print(f'Progress: {i}/{len(images_path)}')

        # If an image_path is given, apply the method only to the image
        # if args.image_path is not None:
        image_path = os.path.join(voc_img_folder, img_name)
        # print(f'images_path: {image_path}')
        # exit()
        dataset = ImageDataset(image_path, args.resize)
        # else:
        ...
        for im_id, inp in enumerate(pbar):
            ...
            if not args.dinoseg:
                ...
                collect_pred_dict['image_files'].append(img_name)
                collect_pred_dict['bboxes'].append(pred.tolist())

                # collect_pred.append(f'{image_path},{int(pred[0])},{int(pred[1])}, {int(pred[2])},{int(pred[3])}')

                print(f'here...pred: {collect_pred_dict}')
                exit()
    ...
    # print(f'dict: {collect_pred_dict}')
    # exit()
    with open(voc_annotation_file, 'w') as f:
        json.dump(collect_pred_dict, f)
        # for item in collect_pred:
        #    json.dump(item, f, ensure_ascii=False)
        #    f.write('\n')
    print(f'Annotation file written successfully.')

    # with open(annotation_file, 'r') as f:
    #     dict = json.load(f)
    #
    # print(f'after reading: {dict.keys()}')
